...
public actor ReactToRoleHandler {
    ...
    struct RequestHandler: Sendable {
        ...
        init(
            ...
            self.logger = logger
            self.guildId = guildId
            self.guildMembersEnabled = await cache?.requestMembers.isEnabled(for: guildId) ?? false
        }

        func cacheWithIntents(_ intents: Gateway.Intent...) async -> DiscordCache? {
            if let cache {
                let cacheIntents = await cache.intents
                if intents.allSatisfy({ cacheIntents.contains($0) }) {
                    return cache
                }
            }
            return nil
        ...
        func getRole(id: RoleSnowflake) async throws -> Role {
            if let cache = await cacheWithIntents(.guilds) {
        ...
        func memberHasRole(roleId: RoleSnowflake, userId: UserSnowflake) async -> Bool {
            if self.guildMembersEnabled,
               let cache = await cacheWithIntents(.guilds, .guildMembers) {
                let guild = await cache.guilds[guildId]
    ...
    public init(
        ...
        self.requestHandler = await .init(
            cache: cache,
